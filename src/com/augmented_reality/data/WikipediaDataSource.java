...
public class WikipediaDataSource extends NetworkDataSource {
    ...
    public String createRequestURL(double lat, double lon, double alt, float radius, String locale) {
        return BASE_URL + "?lat=" + lat + "&lng=" + lon + "&radius=" + radius + "&maxRows=40" + "&lang=" + locale;//+ "&username=demo";
    ...
    private Marker processJSONObject(JSONObject jo) {
        ...
        if (jo.has("title") && jo.has("lat") && jo.has("lng") && jo.has("elevation")) {
            try {
                ma = new IconMarker(jo.getString("title"), jo.getDouble("lat"), jo.getDouble("lng"), jo.getDouble("elevation"), Color.WHITE, icon,null,null);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
